...
const getSocialLinks = () => {
  ...
  if (social.github) {
    const handle = social.github.split("/").pop() || "";
    links.push({
      label: "GitHub",
      href: social.github,
      handle: `@${handle}`,
  ...
  if (social.linkedin) {
    const handle = social.linkedin.split("/").pop() || "";
    links.push({
      label: "LinkedIn",
  ...
  // Add email from config
  const email =
    portfolioConfig.contact?.email ||
    portfolioConfig.socialLinks?.email?.replace("mailto:", "") ||
    "";
  if (email) {
    links.push({
      label: "Email",
