...
// Navigation items - Blog removed, using standard portfolio pages
const navItems = [
  { label: "Home", href: "/" },
...
const getSocialLinks = () => {
  ...
  const social = portfolioConfig.socialLinks || {};

  if (social.github) {
    links.push({ label: "GitHub", href: social.github, icon: Github });
  }
  if (social.twitter) {
    links.push({ label: "Twitter", href: social.twitter, icon: Twitter });
  }
  if (social.linkedin) {
    links.push({ label: "LinkedIn", href: social.linkedin, icon: Linkedin });
  }

  return links;
};

// Get user's initials for logo
...
export function Header() {
  ...
  const initials = getInitials(portfolioConfig.name || "Portfolio");
